...
export NFTSET_CIDR_USER_CFG=$NFTSET_CIDR_CFG
export NFTSET_IP_USER_CFG=$NFTSET_IP_CFG
    ...
    if [ "$ADD_USER_ENTRIES" = "1" ]; then
        if [ -f "$USER_ENTRIES_FILE" ]; then
            $AWK_CMD 'BEGIN {
                    ...
                    END {
                        printf "table %s {\n%s", ENVIRON["NFT_TABLE"], ENVIRON["NFTSET_CIDR_USER_CFG"] >> ENVIRON["IP_DATA_FILE"];
                        delete cidr_array[0];
                        if(length(cidr_array) > 0) {
                            printf "elements={%s};", writeIpList(cidr_array) >> ENVIRON["IP_DATA_FILE"];
                        };
                        printf "}\n%s", ENVIRON["NFTSET_IP_USER_CFG"] >> ENVIRON["IP_DATA_FILE"];
                        delete ip_array[0];
